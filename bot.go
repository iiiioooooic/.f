...
const botToken = "8623563609:AAFqbfFyiCCgaHmxX7XnwcGrrIRnTi4XQAk"
		...
		return func(c tele.Context) error {
			...
			if groupsOnly {
				...
				if isGroup {
					...
					return c.Send("🔒 This group is not authorized. @ArxisX admin to /addgp.")
				}
			}

			// Default: every user must be explicitly allowed
			if !allowedByUser {
				return c.Send("🔒 You are not authorized. Ask admin to /allowuser @ArxisX ID.")
			}
